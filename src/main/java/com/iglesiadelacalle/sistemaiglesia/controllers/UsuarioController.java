...
public class UsuarioController {
    ...
    // ==========================================
    private void registrarHistorial(String accion, String solicitanteUsername, Persona objetivo) {
        String solNombre = "Sistema";
        String solCedula = "N/A";
        
        if (solicitanteUsername != null && !solicitanteUsername.isEmpty()) {
            Optional<Usuario> solOpt = usuarioRepository.findAll().stream()
        ...
        String objNombre = objetivo.getNombre() + " " + objetivo.getApellido();
        String objCedula = objetivo.getCedula() < 0 ? "N/C" : String.valueOf(objetivo.getCedula());

        HistorialUsuario h = new HistorialUsuario();
        h.setAccion(accion);
        h.setSolicitanteNombre(solNombre);
        h.setSolicitanteCedula(solCedula);
        h.setObjetivoNombre(objNombre);
        h.setObjetivoCedula(objCedula);
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> credenciales) {
        try {
            ...
            return ResponseEntity.ok(Map.of(
                ...
                "estado", user.getEstado() != null ? user.getEstado() : "Activo"
            ));
        } catch (Exception e) { 
