...
// ⚡ VARIABLES DE PAGINACIÓN: CUADRÍCULA
let currentPage = 1;
...
async function proceedToggleStatus(userId, currentState) {
    ...
    try {
        const response = await fetch(`http://localhost:8080/api/usuarios/${userId}/cambiar-estado`, {
            ...
            body: JSON.stringify({ 
                estado: nuevoEstado,
                solicitante: sessionData.username 
            })
        });
...
function updateCounters(usuariosActivosEnPantalla) {
    let pastores = 0;
    let lideres = 0;
    let congregantes = 0;
    let noCedulados = 0;

    usuariosActivosEnPantalla.forEach(user => {
        if (user.privilege === 'Pastor') pastores++;
        if (user.privilege === 'Líder') lideres++;
        if (user.privilege === 'Congregante') congregantes++;
        
        if (user.cedula && (user.cedula.toString().startsWith('-') || parseInt(user.cedula) < 0)) {
            noCedulados++;
        }
    });

    const elPastores = document.getElementById('count-pastores');
    const elLideres = document.getElementById('count-lideres');
    const elCongregantes = document.getElementById('count-congregantes');
    const elNoCedulados = document.getElementById('count-nocedulados');

    if(elPastores) elPastores.innerText = pastores;
    if(elLideres) elLideres.innerText = lideres;
    if(elCongregantes) elCongregantes.innerText = congregantes;
    if(elNoCedulados) elNoCedulados.innerText = noCedulados;
}
...
function renderUsers() {
    ...
    if (!paginationContainer) {
        ...
        paginationContainer.className = 'max-w-2xl mx-auto mt-6 mb-10 flex justify-center items-center gap-2';
    ...
    const filteredUsers = usersDatabase.filter(user => {
        ...
        let matchesPrivilege = false;
        if (filterPrivilege === 'Todos') {
            matchesPrivilege = true;
        } else if (filterPrivilege === 'No Cedulado') {
            matchesPrivilege = user.cedula && (user.cedula.toString().startsWith('-') || parseInt(user.cedula) < 0);
        } else {
            matchesPrivilege = user.privilege === filterPrivilege;
        }

        return matchesSearch && matchesNationality && matchesStatus && matchesPrivilege;
    });

    updateCounters(filteredUsers);
    ...
    // ⚡ LÓGICA DE PAGINACIÓN DE 6 EN 6 ⚡
    ...
    paginatedUsers.forEach(user => {
        ...
        let cedulaDisplay = '';
        if (user.cedula && (user.cedula.toString().startsWith('-') || parseInt(user.cedula) < 0)) {
            cedulaDisplay = `<span class="inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-400">NO CEDULADO</span>`;
        } else {
            cedulaDisplay = `<span class="text-sm text-slate-500 dark:text-slate-400">${user.nationality}-${user.cedula}</span>`;
        }
        ...
        card.innerHTML = `
            <div class="flex items-center gap-4">
                ...
                <div class="min-w-0">
                    ...
                    <div class="mt-0.5">${cedulaDisplay}</div>
    ...
    if (totalPages > 1) {
        ...
        prevBtn.className = `w-10 h-10 rounded-xl flex items-center justify-center transition-all ${currentPage === 1 ? 'text-slate-400 bg-slate-100 dark:bg-slate-800 cursor-not-allowed' : 'text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 ios-shadow'}`;
        prevBtn.disabled = currentPage === 1;
        prevBtn.onclick = () => { if(currentPage > 1) { currentPage--; renderUsers(); } };
        paginationContainer.appendChild(prevBtn);

        for (let i = 1; i <= totalPages; i++) {
            const pageBtn = document.createElement('button');
            pageBtn.innerText = i;
            pageBtn.className = `w-10 h-10 rounded-xl text-sm font-bold flex items-center justify-center transition-all ${currentPage === i ? 'bg-primary text-white shadow-md shadow-primary/30' : 'text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 ios-shadow'}`;
            pageBtn.onclick = () => { currentPage = i; renderUsers(); };
            paginationContainer.appendChild(pageBtn);
        }

        const nextBtn = document.createElement('button');
        nextBtn.innerHTML = '<span class="material-icons-round text-sm">chevron_right</span>';
        nextBtn.className = `w-10 h-10 rounded-xl flex items-center justify-center transition-all ${currentPage === totalPages ? 'text-slate-400 bg-slate-100 dark:bg-slate-800 cursor-not-allowed' : 'text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 ios-shadow'}`;
        nextBtn.disabled = currentPage === totalPages;
        nextBtn.onclick = () => { if(currentPage < totalPages) { currentPage++; renderUsers(); } };
...
// --- ⚡ LÓGICA DE HISTORIAL DE USUARIOS (CON PAGINACIÓN) ⚡ ---
...
function renderHistoryTable() {
    ...
    if (historyData.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" class="px-6 py-8 text-center text-slate-500">No hay acciones registradas.</td></tr>`;
        return;
    }
    ...
    paginatedHistory.forEach(reg => {
        ...
        } else if (reg.accion.includes("Bloqueado")) {
        ...
        tr.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="text-[10px] text-slate-500 font-mono mt-0.5">C.I: ${reg.solicitanteCedula}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-xs font-bold text-slate-900 dark:text-white uppercase">${reg.objetivoNombre}</div>
                <div class="text-[10px] text-slate-500 font-mono mt-0.5">C.I: ${reg.objetivoCedula}</div>
            </td>
