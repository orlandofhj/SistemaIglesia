...
function aplicarPermisos() {
    ...
    document.querySelectorAll('.pastor-only').forEach(el => {
        if (rol === 3 || rol === 'Pastor') el.classList.remove('hidden');
    ...
    if (rol === 0 || rol === 1) { 
        Object.values(sidebarLinks).forEach(el => { if (el) el.classList.add('hidden'); });
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 2. FORZAR MIS BLOQUES
    let btnBloques = navMenu.querySelector('a[href="/mis-bloques"]');
    if (!btnBloques) {
        const btnMin = navMenu.querySelector('a[href="/mis-ministerios"]') || botonNoticias;
        btnMin.insertAdjacentHTML('afterend', `
            <a class="${path.includes('/mis-bloques') ? clasesActivo : clasesInactivo}" href="/mis-bloques" id="btn-mis-bloques">
                <span class="material-icons-round">local_library</span> <span>Mis Bloques</span>
            </a>
        `);
        btnBloques = navMenu.querySelector('a[href="/mis-bloques"]') || document.getElementById('btn-mis-bloques');
    }

    // 3. ⚡ VALIDACIÓN INTELIGENTE PARA "MIS TEOLOGÍAS" ⚡
    if (!navMenu.querySelector('a[href="/mis-teologias"]')) {
        try {
            const resClases = await fetch('/api/clases-teologia/todos');
            if (resClases.ok) {
                const clases = await resClases.json();
                
                const nombreS = String(session.nombre || '').toLowerCase().trim();
                const apellidoS = String(session.apellido || '').toLowerCase().trim();
                const nombreCompleto = `${nombreS} ${apellidoS !== 'undefined' ? apellidoS : ''}`.trim();
                
                // Verificamos si el usuario actual es estudiante en teología
                const estaCursandoTeologia = clases.some(c => c.estudiantes && c.estudiantes.some(est => {
                    const estNombre = `${(est.nombre||'')} ${(est.apellido||'')}`.toLowerCase().trim();
                    return estNombre === nombreCompleto || estNombre.includes(nombreS);
                }));

                // Si está cursando, O SI ESTÁ EN LA VISTA ACTUAL, agregamos el botón
                if (estaCursandoTeologia || path.includes('/mis-teologias')) {
                    if (btnBloques) {
                        btnBloques.insertAdjacentHTML('afterend', `
                            <a class="${path.includes('/mis-teologias') ? clasesActivo : clasesInactivo}" href="/mis-teologias">
                                <span class="material-icons-round">menu_book</span> <span>Mis Teologías</span>
                            </a>
                        `);
                    }
                }
            }
        } catch(e) { console.error("No se pudo validar teología:", e); }
    }
});